...
class GroupService {
  ...
  static async leaveGroup(groupId: string, userId: string): Promise<boolean> {
    try {
      ...
      // Safely decrement member_count using read-modify-write to avoid relying on unsupported rpc expression
      try {
        const { data: grp, error: grpErr } = await supabase
          .from("groups")
          .select("member_count")
        ...
        if (!grpErr && grp) {
          const newCount = Math.max(0, (Number(grp.member_count) || 0) - 1);
          await supabase.from("groups").update({ member_count: newCount }).eq("id", groupId);
        }
      } catch (updateErr) {
        console.error("Failed to update member_count after leave:", updateErr);
      }
