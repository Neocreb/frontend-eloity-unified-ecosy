...
const UserViewers: React.FC = () => {
  ...
  useEffect(() => {
    const loadViewers = async () => {
      try {
        setIsLoading(true);
        // TODO: Replace with actual API call to fetch viewers
        // const response = await profileService.getViewers(username);
        // setUsers(response);
        setUsers([]);
      } catch (error) {
        ...
        <div className="px-4 py-3 sm:px-6 space-y-2">
          {isLoading ? (
            <div className="text-center py-12">
              <p className="text-muted-foreground dark:text-slate-400">Loading viewers...</p>
            </div>
            ...
            <div className="text-center py-12">
              ...
              <h3 className="text-lg font-medium mb-2 dark:text-white">No recent profile views</h3>
              <p className="text-muted-foreground text-sm px-4 dark:text-slate-400">
                {searchQuery ? "No users match your search" : "Viewers will appear here when people visit your profile"}
              </p>
            </div>
          ) : (
            filteredUsers.map((user) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0 overflow-hidden">
                  ...
                  {user.lastSeen && (
                    <div className="text-xs text-muted-foreground dark:text-slate-400 flex items-center gap-1 mt-1">
